console.log('Activity 4 (debug_ajax) begins on line 156');
...
// start activity 4: debug_ajax 
// define function debugCallback (this will be called in debugAjax later)
function debugCallback(response){
    // insert the response data into the div with id "myDiv" in index.html
    document.querySelector("#myDiv").insertAdjacentHTML('beforeend', '<br>GeoJSON data:<br>' + JSON.stringify(response))
    // log the response data to the console to check that it is being handled correctly
    console.log(response);
};

// define function debugAjax to first fetch the GeoJSON data from MegaCities.geojson, then convert the response to JSON format, and finally call the debugCallback function to deal with the JSON data
function debugAjax(){
    // fetch the GeoJSON data from MegaCities.geojson (in the Chapter03/data folder)
	fetch("data/MegaCities.geojson")
        // the first .then() converts the response (the raw data from the fetch request) to JSON format
        .then(function(response){
            return response.json();
        })
        // the second .then() calls the debugCallback function which will insert the converted json data (the response from the first .then()) into the html and log it to the console
		.then(function(response){
			debugCallback(response);
        })
};
// finally, call the debugAjax function when the window loads to start the process of fetching and handling the GeoJSON data
